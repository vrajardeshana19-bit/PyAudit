...
def start_interactive_chat(local: bool = False):
    ...
    console.print(Panel(
        ...
        f"Powered by: [yellow]{mode_title}[/yellow]\n"
        f"Type [bold red]'exit'[/bold red] or [bold red]'quit'[/bold red] to end the chat session.",
        title="PyAudit DevSecOps Chatbot",
    ...
    while True:
        ...
        if user_input.strip().lower() in ["exit", "quit"]:
            console.print("[bold yellow]Ending AI chat session. Stay secure! 🔒[/bold yellow]\n")
            break
        ...
        system_context = (
            "You are PyAudit AI, an expert DevSecOps and Python cybersecurity advisor. "
            "Help the user understand code security vulnerabilities, refactoring fixes, and MITRE ATT&CK concepts concisely.\n\n"
        ...
        if local:
            try:
                ...
                if response.status_code == 200:
                    ...
                    console.print(Panel(ans, title="🦙 PyAudit AI (Local)", border_style="cyan"))
                else:
        ...
        # CLOUD MODE (Google GenAI SDK)
            ...
            try:
                ...
                if res and res.text:
                    console.print(Panel(res.text, title="🤖 PyAudit AI (Cloud)", border_style="magenta"))
                else:
